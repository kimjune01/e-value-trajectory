...
- **(d) AR(1) with drift**: X_t = 0.9·X_{t-1} + 0.03 + ε_t, ε ~ N(0,1). Stationary mean = 0.03/0.1 = 0.3, matching condition (a). Autocorrelated, mean-reverting, no explicit cycle. Tests whether autocorrelation alone produces oscillation artifacts. **Note:** the i.i.d. e-value formula is intentionally misspecified here — the supermartingale guarantee does not hold for dependent data. This is deliberate: the question is what happens when a practitioner naively applies the diagnostic to autocorrelated data. Any spectral artifact in (d) is partly an artifact of the misspecification, which is itself informative (it bounds how much autocorrelation can fool the diagnostic).
